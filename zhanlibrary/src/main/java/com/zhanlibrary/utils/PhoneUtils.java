...
public class PhoneUtils {
    ...
    public static boolean isPhoneNum(String content) {
        if (GHSStringUtil.isEmpty(content)) return false;
     ...
     * @return 135****5467 的样式
     */
    public static String getWorkedNum(String originNum) {
        if (null != originNum && originNum.length() > 8) {
            originNum = originNum.substring(0, originNum.length() - 8) + "****" + originNum.substring(originNum.length() - 4, originNum.length());
        }
        return originNum;
    }
}
