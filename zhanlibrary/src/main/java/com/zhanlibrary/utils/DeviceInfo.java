...
public class DeviceInfo {
    ...
    public static String getDeviceId(Context context) {
        ...
        // The unique identification associated load all GSM and UMTS network
        // mobile phone users
        try{
            imei = tm.getDeviceId();
        }catch (Exception e){
            imei=SpUtils.get(context,"device_id","").toString();
            if("".equals(imei)){
                imei=System.currentTimeMillis()+"88";
                SpUtils.put(context,"device_id",imei);
            }
    ...
    private static boolean isValidIMEI(String imei) {
        ...
        if (cleanedIMEI.length() == 0) {
            return false;
        }

        return cleanedIMEI.matches("^[A-Za-z0-9]$");
    }
