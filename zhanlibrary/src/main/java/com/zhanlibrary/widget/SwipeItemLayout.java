...
public class SwipeItemLayout extends FrameLayout {
	...
	private Btn1OnClickListener listener;
	...
	public View myFindViewById(int id){
		if(contentView!=null){
	...
	public boolean onSwipe(MotionEvent event,int position) {
		...
		case MotionEvent.ACTION_UP:
			if ((mDownX - event.getX()) > (menuView.getWidth() / 2)) {
				// open
				smoothOpenMenu();
				return true;//打开后消费掉事件
			} else if(mDownX>contentView.getWidth()-menuView.getWidth()&&(int)(mDownX - event.getX())==0){//点击btn1了
					if(listener!=null){
...
//		bringChildToFront(mContentView);
	...
	public interface Btn1OnClickListener {//btn1 的点击事件
		void onClick(int position);
	}
	public void setMyOnTouchListener(Btn1OnClickListener listener){
