...
public class MyTextView extends TextView {
     ...
     * 存储当前文本内容，每个item为一行
     */
    public ArrayList<LINE> contentList = new ArrayList<LINE>();
    ...
    public MyTextView(Context context) {
        ...
        lineSpacingDP = DensityUtil.dip2px(context, lineSpacingDP);
        minHeight = 0;
    ...
    public MyTextView(Context context, AttributeSet attrs) {
        ...
        paint.setAntiAlias(true);
        minHeight = 0;
        maxlines = attrs.getAttributeIntValue("http://schemas.android.com/apk/res/android", "maxLines", 100);
        displayMetrics = new DisplayMetrics();
        lineSpacingDP = DensityUtil.dip2px(context, lineSpacingDP);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //        }
        int lineNum;
        if (maxlines > contentList.size()) {
            lineNum = contentList.size();
        } else {
            lineNum = maxlines;
        }
        float height = 0;
        for (int i = 0; i < lineNum; i++) {
            ...
            for (int j = 0; j < line.line.size(); j++) {
                ...
                if (ob instanceof String) {
                    ...
                    int top = (int) height+lineSpacingDP;
                    int right = (int) (realDrawedWidth + width);
                    int bottom = (int) (height + line.height)+lineSpacingDP;
                    d.setBounds(left, top, right, bottom);
                    d.draw(canvas);
                    realDrawedWidth += width;
                }
            }
            height += line.height + lineSpacingDP;
        }
    }
    ...
    private int measureContentHeight(int width) {
        ...
        if (line != null && line.line.size() > 0) {
            contentList.add(line);
            height += lineHeight + lineSpacingDP;
        }
        if (contentList.size() <= 1) {
            oneLineWidth = (int) drawedWidth + leftPadding + rightPadding;
            height = lineSpacingDP + lineHeight + lineSpacingDP;
        }
        return (int) height;
    }
    ...
    public void setLineSpacingDP(int lineSpacingDP) {
        this.lineSpacingDP = DensityUtil.dip2px(context, lineSpacingDP);
    }
}
